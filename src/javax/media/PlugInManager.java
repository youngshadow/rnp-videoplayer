...
{
	// This class is expected to route all calls (using reflection) to javax.media.pim.PlugInManager, see the JavaDoc.
	// We'll do the same, to ensure maximum compatibility, although the reasons for this technique are not clear.
	// I suspect that they did this to make it easier to ship "performance packs" for different operating systems, which might
	...
	private static synchronized boolean init()
	{
		...
		try
		{
			...
			getPlugInListMethod = getStaticMethodOnImplClass("getPlugInList", new Class[]{Format.class, Format.class, int.class}, Vector.class);
			setPlugInListMethod = getStaticMethodOnImplClass("setPlugInList", new Class[]{Vector.class, int.class}, void.class);
			commitMethod = getStaticMethodOnImplClass("commit", new Class[]{}, void.class);
			addPlugInMethod = getStaticMethodOnImplClass("addPlugIn", new Class[]{String.class, Format[].class, Format[].class, int.class}, boolean.class);
			removePlugInMethod = getStaticMethodOnImplClass("removePlugIn", new Class[]{String.class, int.class}, boolean.class);
			getSupportedInputFormatsMethod = getStaticMethodOnImplClass("getSupportedInputFormats", new Class[]{String.class, int.class}, Format[].class);
			getSupportedOutputFormatsMethod = getStaticMethodOnImplClass("getSupportedOutputFormats", new Class[]{String.class, int.class}, Format[].class);
	...
	public static Vector getPlugInList(Format input, Format output, int type)
	{
		if (!init())
			return null;
		return (Vector) callImpl(getPlugInListMethod, new Object[]{input, output, new Integer(type)});	
	}
	...
	public static void setPlugInList(Vector plugins, int type)
	{
		if (!init())
			return;
		callImpl(setPlugInListMethod, new Object[]{plugins, new Integer(type)});	
	}
	...
	public static boolean addPlugIn(String classname, Format[] in, Format[] out, int type)
	{
		if (!init())
			return false;
		return ((Boolean) callImpl(addPlugInMethod, new Object[]{classname, in, out, new Integer(type)})).booleanValue();	
	...
	public static boolean removePlugIn(String classname, int type)
	{
		if (!init())
			return false;
		return ((Boolean) callImpl(removePlugInMethod, new Object[]{classname, new Integer(type)})).booleanValue();	
	}
	...
	public static Format[] getSupportedInputFormats(String className, int type)
	{
		if (!init())
			return null;
		return (Format[]) callImpl(getSupportedInputFormatsMethod, new Object[]{className, new Integer(type)});	
	}
	...
	public static Format[] getSupportedOutputFormats(String className, int type)
	{
		if (!init())
			return null;
		return (Format[]) callImpl(getSupportedOutputFormatsMethod, new Object[]{className, new Integer(type)});	

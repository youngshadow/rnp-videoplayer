...
public class IndexedColorFormat extends VideoFormat
{
	...
	public Object clone()
	{
		return new IndexedColorFormat(FormatUtils.clone(size), maxDataLength, dataType, frameRate,
				lineStride, mapSize, redValues, greenValues,
				blueValues);
	}
	...
	{
		super.copy(f);
		final IndexedColorFormat oCast = (IndexedColorFormat) f;	// it has to be a IndexedColorFormat, or ClassCastException will be thrown.
	...
	public boolean equals(Object format)
	{
		...
		return 
			this.lineStride == oCast.lineStride &&
			this.mapSize == oCast.mapSize &&
			this.redValues == oCast.redValues &&	// strange, compare arrays using ==.  This makes it so that serializing and deserializing this obj results in an obj that is not "equal"
			this.greenValues == oCast.greenValues &&
			this.blueValues == oCast.blueValues;	
	}
	...
	public boolean matches(Format format)
	{
		...
		final boolean result = 
			FormatUtils.matches(oCast.lineStride, this.lineStride) &&

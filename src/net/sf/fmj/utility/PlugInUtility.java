 ...
 * The rest is done by reflection/instantiation/querying.
 * @author Ken Larson
 *
 */
	...
	public static boolean registerPlugIn(String className)
	{
		...
		try
		{
			...
			if (o instanceof Demultiplexer)
			{
				...
				return PlugInManager.addPlugIn(className, formats, new Format[] {}, PlugInManager.DEMULTIPLEXER);
			...
			else if (o instanceof Codec)
			{
				...
				return PlugInManager.addPlugIn(className, inputFormats, outputFormats, (o instanceof Effect) ? PlugInManager.EFFECT : PlugInManager.CODEC);
			...
			else if (o instanceof Renderer)
			{
				...
				final Format[] inputFormats = oCast.getSupportedInputFormats();
				return PlugInManager.addPlugIn(className, inputFormats, new Format[] {}, PlugInManager.RENDERER);
			}
			...
			{	
				...
				return PlugInManager.addPlugIn(className, new Format[] {}, oCast.getSupportedOutputContentDescriptors(null), PlugInManager.MULTIPLEXER);
			}
			else
			{
		...
		catch (Throwable e)	// catch Throwable instead of Exception because some plug-ins load native libraries, and if the native libraries are missing, we can get exceptions not caught by Exception
		{

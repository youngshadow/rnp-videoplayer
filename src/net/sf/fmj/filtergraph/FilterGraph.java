...
//						}
	...
	private static FilterGraphNode findCodecPathTo(int destPlugInType, final Format f, final PlugIn from, final Multiplexer mux, final Format muxInputFormat, final int muxDestTrack, final int depth)
	{
		...
		if (BEST_FIRST_SEARCH_DEPTH > 0)
		{
			...
			for (int cutoffDepth = 0; cutoffDepth < BEST_FIRST_SEARCH_DEPTH; ++cutoffDepth)
			{
				...
				FilterGraphNode n = findCodecPathTo(destPlugInType, f, from, new HashSet(), mux, muxInputFormat, muxDestTrack, depth, cutoffDepth, BEST_FIRST_SEARCH_BREADTH);
				if (n != null)
				{	
	...
	private static FilterGraphNode findCodecPathTo(int destPlugInType, final Format f, final PlugIn from, final Set excludeFormatAtDepths, final Multiplexer mux, final Format muxInputFormat, final int muxDestTrack, final int depth, final int cutoffDepth, final int maxBestCodecs)
	{
		if (f == null)
		...
		{
			final Format fAccepted = negotiate(f, mux, muxInputFormat, muxDestTrack, from);
			if (fAccepted != null)
			{	
	    ...
	    for (int j = 0; j < codecs.size(); ++j)
	    {
	    	...
	    	// TODO: should we call setInputFormat here, or below? (as we are doing now)
...
//	    	}
	    	...
	    	for (int codecOutputFormatIndex = 0; codecOutputFormatIndex < codecOutputFormats.length; ++codecOutputFormatIndex)
	    	{
	    		...
	    		if (TRACE) logger.finest(indent(depth) + "Found Codec output format: " + codecOutputFormat);
	    	}
	    	for (int codecOutputFormatIndex = 0; codecOutputFormatIndex < codecOutputFormats.length; ++codecOutputFormatIndex)
	    	{
	    		final Format codecOutputFormat = codecOutputFormats[codecOutputFormatIndex];
...
//		    }
		    ...
		    for (int j = 0; j < codecFormatPairs.size(); ++j)
		    {
		    	...
		    	if (TRACE) logger.finer(indent(depth) + j + ". Will try " + codec.getClass().getName() + " with output format: " + codecOutputFormat);
	    ...
	    for (int j = 0; j < codecFormatPairs.size(); ++j)
	    {
	    	...
	    	if (TRACE) logger.finer(indent(depth) + "Trying " + codec.getClass().getName() + " with output format: " + codecOutputFormat);
	    	{
	    		...
	    		final Format codecOutputFormatAccepted = codec.setOutputFormat(codecOutputFormat);	// TODO: this should be set only in negotiation
	    		...
	    		final FilterGraphNode tail = findCodecPathTo(destPlugInType, codecOutputFormatAccepted, codec, excludeFormatAtDepths, mux, muxInputFormat, muxDestTrack, depth + 1, cutoffDepth, maxBestCodecs);
	    		if (tail != null)
	    		{	
...
//	}
	...
	{
		return buildGraphTo(PlugInManager.RENDERER, contentDescriptor, demux, null, null, -1);
	}
	
	public static DemuxNode buildGraphToMux(ContentDescriptor contentDescriptor, Demultiplexer demux, final Multiplexer mux, final Format muxInputFormat, final int muxDestTrack)
	{
		return buildGraphTo(PlugInManager.MULTIPLEXER, contentDescriptor, demux, mux, muxInputFormat, muxDestTrack);
	}
	
	// TODO: throw exception instead of returning null;
	/** demux should be opened and started before calling */
	private static DemuxNode buildGraphTo(int pluginType, ContentDescriptor contentDescriptor, Demultiplexer demux, final Multiplexer mux, final Format muxInputFormat, final int muxDestTrack)
	{
			...
			if (TRACE) logger.fine("Number of tracks: " + demuxNode.getTracks().length);
			...
			for (int trackIndex = 0; trackIndex < demuxNode.getTracks().length; ++trackIndex)
			{
				...
				if (TRACE) logger.fine("Track format: " + t.getFormat());
...
//				}
				...
				final FilterGraphNode n = findCodecPathTo(pluginType, t.getFormat(), demux, mux, muxInputFormat, muxDestTrack, 0);	// TODO: specify which track it is from?
				if (n == null)
			...
			{	
				...
				return demuxNode;	// TODO: we really only want to return true if ALL tracks are complete.
			}

	...
	private class PullSourceStreamTrack extends AbstractTrack
	{
		...
		public PullSourceStreamTrack(PullSourceStream pssForFormat, PullSourceStream pssForReadFrame) throws UnsupportedAudioFileException, IOException
		{
			...
			// determine format and frame size.
			{
				...
				this.format = JavaSoundRenderer.convertFormat(javaSoundInputFormat);
...
//			Note: This scenario is necessary only if a PullDataSource Demultiplexer implementation wants to avoid buffering data locally and copying the data to the Buffer passed in as a parameter. Implementations might decide to buffer data and not block (if possible) and incur data copy overhead. 
		...
		public void readFrame(Buffer buffer)
		{
			...
			final int BUFFER_SIZE = 10000;	// TODO: how do we determine this size?  Is this what the BufferControl control is for?
		...
		{
			return TIME_UNKNOWN;	// this is what audio tracks are supposed to return
		}

		public int mapTimeToFrame(Time t)
		{	
			return FRAME_UNKNOWN;		// this is what audio tracks are supposed to return
		}
		...
		{
			// TODO: for ogg/mp3, we end up with -1 frame length and return unknown.  Maybe only the codec can figure this out.
			// or maybe we need to open our own converted stream to find the length.
	...
	// AudioSystem.getAudioInputStream needs to use marks to determine the format.

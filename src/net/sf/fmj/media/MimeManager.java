 ...
 *
 */
public class MimeManager // not final so we can extend it to implement one in com.sun.media
{
	...
	static
	{
		...
		put("aif", "audio/x_aiff");	// reordered so it won't be the default in reverse lookup
		put("aiff", "audio/x_aiff");
		put("midi", "audio/midi");
		put("jmx", "application/x_jmx");
		put("mpv", "video/mpeg");	// reordered so it won't be the default in reverse lookup
		put("mpg", "video/mpeg");
//		put("aif", "audio/x_aiff");
		put("wav", "audio/x_wav");
		put("mp3", "audio/mpeg");
		put("mpa", "audio/mpeg");	// reordered so it won't be the default in reverse lookup
		put("mp2", "audio/mpeg");
		//put("mpa", "audio/mpeg");
...
//		put("mpv", "video/mpeg");
		...
		if (!jmfDefaults)
		{
			// see http://wiki.xiph.org/index.php/MIME_Types_and_File_Extensions for ogg extensions.
			
			put("ogg", "audio/ogg"); // this is somewhat problematic, since .ogg extension is often used for audio-only files, but is also used for video files.
			put("ogx", "application/ogg");
			put("oga", "audio/ogg");
			put("ogv", "video/ogg");

			// include other types from the xiph wiki, regardless of whether we actually support them:
			put("spx", "audio/ogg");
			...
			put("wmv", "video/x-ms-wmv");	// this is incorrectly specified as audio/... on the above link.  Gnome desktop has this as video/x-ms-asf
	...
	{	
		String result = (String) Registry.getInstance().getMimeType(fileExtension);
		if (result != null)
			return result;

	...
	static class FrameAssembler
	{
		...
		public int copyToBuffer(Buffer bDest)
		{
			...
			{	
				...
				offsetAfterHeaders = RFC2035.MakeHeaders(false, data, offsetAfterHeaders, jpegRtpHeaderFirst.getType(), jpegRtpHeaderFirst.getQ(), jpegRtpHeaderFirst.getWidthInBlocks(), jpegRtpHeaderFirst.getHeightInBlocks());
			}
...
//				if (TRACE) System.out.println("End copying.");
				...
				System.arraycopy((byte []) b.getData(), b.getOffset() + JpegRTPHeader.HEADER_SIZE, data, offsetAfterHeaders + jpegRtpHeader.getFragmentOffset(), b.getLength() - JpegRTPHeader.HEADER_SIZE);
			}
	...
	private static class FrameAssemblerCollection
	{
		...
		public void removeOlderThan(long timestamp)
		{
			...
			while (i.hasNext())
			{
				...
				{	if (TRACE) System.out.println("Discarding incomplete frame older than " + timestamp + ", ts=" + entryTimestamp);
					i.remove();
				}
			}
		...
		public void removeAllButNewestN(int n)
		{
			...
			{	
				...
				if (TRACE) System.out.println("Discarding " + completeIncomplete + " frame (not in newest " + n + ", ts=" + oldestTimestamp);

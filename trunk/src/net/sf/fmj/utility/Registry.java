 ...
 * The registry may be serialized to an XML.  The XML file is nominally located in ${user.home}/.fmj.registry.xml
 * 
 * This object is used by the PackageManager and the PluginManager for persisting data across sessions.
 * 
 * Currently the Registry does not store the supported input and output formats for Plugins.
 * This may be supported by adding CDATA sections that are serialized Format objects.
 * However, it would be good to be able to clear the stored formats, and refresh the supported
 * formats by introspecting the Plugins.  Sometimes the installed plugins may be updated, and the
 * list of supported formats may change for the same plugin class.
 * 
 * Nevertheless, the present situation is that the PluginManager will need to determine supported formats upon
 * loading informatin from this Registry.
 * 
 * TODO separate the persistence mechanism from this object, so that it may be updated/plugged-in.
 * TODO perhaps remove reliance on JDOM.  Although JDOM makes it easy to program, it is another jar to ship.
 * 
...
public class Registry {
	 ...
	 * 
	 * Protocols are converted to package names, e.g. "http" -> "http"
	 ...
	 * These package names are added to the prefices in this list to determine
	 * Handlers for them.
	 * i.e. "<i>prefix</i>.media.content.text.html.Handler"
	 */
	...
	private void setDefaults() {
		...
		// FMJ's prefix can be not added by default if the system property net.sf.fmj.utility.JmfRegistry.JMFDefaults is set to true;
		...
		final int flags = jmfDefaults ? RegistryDefaults.JMF : RegistryDefaults.ALL;
	...
	private File getRegistryFile() {
		...
		final String filename = System.getProperty("net.sf.fmj.utility.JmfRegistry.filename", ".fmj.registry.xml"); // allow override

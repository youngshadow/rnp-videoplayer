...
/**
 * 
 * Compares formats to a target format.  Used to improve search order in building filter graph, when a target format is known.
 * @author Ken Larson
 *
 */
public class FormatProximityComparator implements Comparator
{
	...
	public int compare(Object a, Object b)
	{
		...
		// similarity is higher for better matches, but we need lower values to sort right, so we multiply by -1.
	...
	{
		int n = 0;
		n += similarity(a.getSampleRate(), b.getSampleRate(), SAMPLE_RATE_EQUAL, SAMPLE_RATE_MATCHES);
		n += similarity(a.getSampleSizeInBits(), b.getSampleSizeInBits(), SAMPLE_SIZE_IN_BITS_EQUAL, SAMPLE_SIZE_IN_BITS_MATCHES);
		n += similarity(a.getChannels(), b.getChannels(), CHANNELS_EQUAL, CHANNELS_MATCHES);
		n += similarity(a.getEndian(), b.getEndian(), ENDIAN_EQUAL, ENDIAN_MATCHES);
		n += similarity(a.getSigned(), b.getSigned(), SIGNED_EQUAL, SIGNED_MATCHES);
		n += similarity(a.getFrameSizeInBits(), b.getFrameSizeInBits(), FRAME_SIZE_IN_BITS_EQUAL, FRAME_SIZE_IN_BITS_MATCHES);
		n += similarity(a.getFrameRate(), b.getFrameRate(), FRAME_RATE_EQUAL, FRAME_RATE_MATCHES);
		return n;

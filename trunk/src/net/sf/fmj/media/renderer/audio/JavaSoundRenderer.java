...
//		return (Format[]) supportedFormats.toArray(new Format[]{});
	...
	public int process(Buffer buffer) {
		...
		while (length > 0)
		{
			...
			final int n = sourceLine.write(data, offset, length);		// TODO: this can block for a very long time if it doesn't
			if (n >= length)
				break;
			else if (n == 0)
			{
...
//				}
	...
	public int hashCode() {
		return super.hashCode() ^ 0xAD;	// TODO: this hash code change is useless.
...
//		 Already hash value of 1262232571547748861 in plugin table for class name of com.sun.media.renderer.audio.JavaSoundRenderer
	...
	public static AudioFormat convertFormat(javax.sound.sampled.AudioFormat format) {
		...
		AudioFormat jmfFormat = new AudioFormat(
				encodingString,
				(double) sampleRate,
				sampleSize,
				channels,
	...
	{
		// TODO: perhaps we should use reflection to avoid class not found problems if javazoom is not in the classpath.

		final Encoding[] mpegEncodings = new Encoding[] {
	...
	private static Encoding toVorbisEncoding(String encodingStr)
	{
		...
		final Encoding[] vorbisEncodings = new Encoding[] {

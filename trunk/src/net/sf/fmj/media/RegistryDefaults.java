...
/**
 * Defaults for the FMJ registry.  Broken out into fmj, jmf, and third-party.
 * fmj ones are fmj-specific.
 * jmf ones are to duplicate what is in jmf (useful if jmf is in the classpath).
 * third-party ones are those that are not included with fmj but might be in the classpath
 * (like fobs4jmf).
 * The flags give us the flexibility to make the registry the same as JMF's or JMF's + FMJ's, or just FMJ's.
 * Making it the same as JMF's is useful for unit testing.
 * @author Ken Larson
 *
 */
public class RegistryDefaults
{
	...
	public static List<String> protocolPrefixList(int flags)
	{
		...
		if ((flags & FMJ) != 0)
		{
			...
			if (OSUtils.isLinux())	// TODO: we could add these for other OS's, as gstreamer is cross-platform.
			{
				// GStreamer:
		...
		{
			protocolPrefixList.add("com.omnividea");	// FOBS4JMF: may not be in classpath
		}
...
//		 Already hash value of 8706141154469562557 in plugin table for class name of com.sun.media.codec.video.jpeg.Packetizer
//		Problem adding net.sf.fmj.media.codec.video.jpeg.DePacketizer to plugin table.
//		 Already hash value of 3049617401990556986 in plugin table for class name of com.sun.media.codec.video.jpeg.DePacketizer
...
//		 Already hash value of -2095741743343195187 in plugin table for class name of com.sun.media.multiplexer.RTPSyncBufferMux
...
//		 TODO: this causes audio not to play properly:
		if ((flags & FMJ) != 0)
		{			PlugInManager.addPlugIn("net.sf.fmj.media.parser.RawPushBufferParser", 
						new Format[] {
	...
	//		 PlugInManager.CODEC:
		...
		if ((flags & FMJ) != 0)
		{	PlugInManager.addPlugIn("net.sf.fmj.media.codec.video.jpeg.Packetizer", 
					new Format[] {
		...
		if ((flags & FMJ) != 0)
		{	PlugInManager.addPlugIn("net.sf.fmj.media.codec.video.jpeg.DePacketizer", 
					new Format[] {
						new VideoFormat("jpeg/rtp", null, -1, Format.byteArray, -1.0f),
					}, 
					new Format[] {
	...
	public static void unRegisterPlugins(int flags)
	{
		...
		for (int type : types)
		{
			...
			for (Object o : v)
			{
				...
				if ((flags & JMF) != 0)
				{
					if (className.startsWith("com.ibm.") || className.startsWith("com.sun.") || className.startsWith("javax.media."))
						remove = true;
				}
				if ((flags & FMJ) != 0)
				{

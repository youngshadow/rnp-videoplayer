...
public class ContainerPlayer implements TransportControl, SourcedTimerListener, TimeSource {
    ...
    private Component visualComponent, controle;
        ...
        public void controllerUpdate(ControllerEvent event) {
           ...
           Player  player = (Player) event.getSourceController();
            ...
            if (event instanceof RealizeCompleteEvent) {
                ...
                //    System.out.println("size: " + player.getVisualComponent().getHeight() + " == " + player.getVisualComponent().getSize().getSize().getHeight());
                    setControll(player.getControlPanelComponent());
                ...
                transportControlState.setAllowVolume(true);
...
//		    player.syncStart(tb.getTime());
   ...
   //     System.out.println("1-" + player.getVisualComponent().getPreferredSize());
...
//        transportControlState.setAllowVolume(true);
    ...
    private void setVisualComponent(final Component newVisualComponent) {
        if (this.getVisualComponent() == newVisualComponent) {
            return;
        }
        ...
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                if (ContainerPlayer.this.getVisualComponent() != null) {
                    getContainer().remove(ContainerPlayer.this.getVisualComponent());
                }
                ContainerPlayer.this.visualComponent = newVisualComponent;
                if (newVisualComponent != null) {
                    getContainer().add(newVisualComponent);
                    // getContainer().add(getControll(),BorderLayout.SOUTH);
                }
    ...
    public Component getControll() {
        return controle;
    }

    public void setControll(Component controll) {
    ...
    public int[] getDimensao() {
        if (player != null) {
            ...
            Dimension size = ContainerPlayer.this.player.getVisualComponent().getPreferredSize();
            videoWidth = size.width;
            videoHeight = size.height;
            int[] valores = new int[2];
            valores[0] = (int) (videoWidth - 8);
            valores[1] = (int) (videoHeight - 34);
            return valores;
        }
        return new int[]{0, 0};
    }
